...
test.describe("Function call frame to variable assignment frame transformation", () => {
    test("Just have \"=\"", async ({page}) => {
        // Make a function call (" c" opens the frame-commands pane then selects "function call",
        // its own shortcut being "c" -- a single Space no longer inserts one directly, see
    ...
    test("Have \"a=\"", async ({page}) => {
        // Make a function call, then explicitly type its call's opening bracket -- the closing
        // one appears automatically -- and step back out of it before typing "=", to match where
        // bare typing used to leave the cursor when its "()" was pre-filled: bare-typed func-call
        // frames no longer get that (unlike the explicit "c" pane command above), see 23a735f1.
    ...
    test("Have \"abc123\" to \"abc=123\"", async ({page}) => {
        // Make a function call and "abc123", then explicitly type its call's opening bracket and
        // step back out of it (see the comment in "Have \"a=\"" above):
    ...
    test("Have \"ab==456\" to \"a=b=456\"", async ({page}) => {
        // Make a function call and "ab456", then explicitly type its call's opening bracket and
        // step back out of it (see the comment in "Have \"a=\"" above):
