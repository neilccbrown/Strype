    ...
    <div class="file-system-pane">
        ...
        <template v-else>
            <div class="file-system-pane-root" v-if="dataRoot">
                ...
                <FileSystemTree :node="dataRoot" start-expanded @download="onDownload" />
            </div>
            <div class="file-system-pane-root" v-if="localRoot">
                <h4>{{ $t("fileSystemTab.local") }}</h4>
                <FileSystemTree :node="localRoot" start-expanded @download="onDownload" />
            </div>
        </template>
    </div>
</template>
...
export default defineComponent({
    ...
    methods: {
        ...
        onDownload(node: FsTreeNode): void {
            void downloadFsFile(node.path, node.name);
        },
    },
});
</script>
