    ...
    created() {
        ...
        window.addEventListener(
            ...
            (event: KeyboardEvent) => {
                ...
                if (!isEditing && !(isPythonExecuting && ((this.$refs.strypePEA as InstanceType<typeof PythonExecutionArea>).$data.isTurtleListeningKeyEvents || (this.$refs.strypePEA as InstanceType<typeof PythonExecutionArea>).$data.peaDisplayTabIndex == 2)) && ["ArrowDown", "ArrowUp", "ArrowLeft", "ArrowRight", "Tab", "Home", "End"].includes(event.key)) {
                    event.stopImmediatePropagation();
                    event.stopPropagation();
                    event.preventDefault();
                ...
                if(event.key != "Escape"){
                    ...
                    else if(isPythonExecuting && (this.$refs.strypePEA as InstanceType<typeof PythonExecutionArea>).$data.peaDisplayTabIndex != 2){
