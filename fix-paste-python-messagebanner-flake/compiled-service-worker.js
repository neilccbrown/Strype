...
//#endregion
//#region src/workers/service-worker.ts
		...
		return (() => {
			...
			function i() {
				...
				return (n) => {
					...
					return !!s(i) && (n.respondWith(function() {
						return r(this, void 0, void 0, (function* () {
							...
							if (i.endsWith("/write")) {
								const { message: o, messageId: s } = yield n.request.json(), i = t[s];
								return i ? (i(r(o)), delete t[s]) : e[s] = o, r({ early: !i });
							}
							if (i.endsWith("/version")) return new Response(o, { status: 200 });
						}));
					}()), !0);
				};
			}
			...
			class c extends Error {
				constructor(e, t) {
					super(`Received status ${t} from ${e}. Ensure the service worker is registered and active.`), this.url = e, this.status = t, this.type = "ServiceWorkerError", Object.setPrototypeOf(this, c.prototype);
				}
			}
			function a(e, t) {
				const r = new TextEncoder().encode(JSON.stringify(t)), { data: n, meta: o } = e;
				if (r.length > n.length) throw new Error("Message is too big, increase bufferSize when making channel.");
				n.set(r, 0), Atomics.store(o, 0, r.length), Atomics.store(o, 1, 1), Atomics.notify(o, 1);
			}
			function f(e, t, n) {
				return r(this, void 0, void 0, (function* () {
					yield navigator.serviceWorker.ready;
					const r = e.baseUrl + "/write", s = Date.now();
					for (;;) {
						const a = yield fetch(r, {
							method: "POST",
							body: JSON.stringify({
								message: t,
								messageId: n
			...
			function l(e = {}) {
				return "undefined" != typeof SharedArrayBuffer ? m(e.atomics) : "serviceWorker" in navigator ? y(e.serviceWorker) : null;
			}
			function m({ bufferSize: e } = {}) {
				return {
					type: "atomics",
					data: new Uint8Array(new SharedArrayBuffer(e || 131072)),
					meta: new Int32Array(new SharedArrayBuffer(2 * Int32Array.BYTES_PER_ELEMENT))
				};
			}
			function y(e = {}) {
				return {
			...
			function h(e, t, { checkInterrupt: r, checkTimeout: n, timeout: s } = {}) {
				...
				if ("atomics" === e.type) {
					...
					a = () => {
						...
						{
							const e = Atomics.exchange(r, 0, 0), n = t.slice(0, e);
							Atomics.store(r, 1, 0);
					...
					const r = new XMLHttpRequest(), s = e.baseUrl + "/read";
					r.open("POST", s, !1);
					const u = {
						messageId: t,
						timeout: n
			...
			function g(e, t) {
				if (e = p(e, 0)) if ("undefined" != typeof SharedArrayBuffer) {
					const t = new Int32Array(new SharedArrayBuffer(Int32Array.BYTES_PER_ELEMENT));
...
self.addEventListener("message", (event) => {
	if (event.data === "claim") self.clients.claim().then(() => {
		event.source?.postMessage("claimed");
	});
});
var syncMessageFetchListener = (0, import_dist.serviceWorkerFetchListener)();
self.addEventListener("fetch", (e) => {
	if (e instanceof FetchEvent) syncMessageFetchListener(e);
});
//#endregion
